...
from .forms import SessionForm, CustomUserCreationForm,CoachNotesForm
...
def dashboard(request):
    ...
    if user.is_coach or user.is_superuser:
        upcoming_sessions = Session.objects.filter(
            timeslot__date__gt=current_time.date()
        ...
        past_sessions = Session.objects.filter(
            timeslot__date__lt=current_time.date()
        ) | Session.objects.filter(
            timeslot__date=current_time.date(),
    ...
    return render(request, template, {
        'upcoming_sessions': upcoming_sessions.order_by('timeslot__date', 'timeslot__start_time'),
        'past_sessions': past_sessions.order_by('-timeslot__date', '-timeslot__start_time'),
    })
...
def make_appointment(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            messages.success(request, "Your session is successfully booked.")
            return redirect('dashboard')
        else:
            messages.error(request, "No available timeslot.")
    else:
        form = SessionForm()
...
def timeslot_calendar_view(request):
    ...
    return render(request, 'calendar/timeslot_list.html', {
        'form': form,
        'timeslots': timeslots,
